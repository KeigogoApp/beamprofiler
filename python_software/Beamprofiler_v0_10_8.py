...
class GUI:
    def setup():
        ...
        root.iconbitmap(os.path.join(exedirpath, "icon_32.ico"))
        ...
        GUI.button(buttonframe, exedirpath)
    ...
    def change_graph_layout(graph_w, graph_h, dpi):
    ...
    def button(frame, exedirpath):
        ...
        darkphoto = tk.PhotoImage(file=os.path.join(exedirpath, "offset.png"))
        ...
        axisphoto = tk.PhotoImage(file=os.path.join(exedirpath, "axis_px.png"))
        ...
        pausephoto = tk.PhotoImage(file=os.path.join(exedirpath, "pause.png"))
        ...
        intensityphoto = tk.PhotoImage(file=os.path.join(exedirpath, "intensity.png"))
        ...
        savephoto = tk.PhotoImage(file=os.path.join(exedirpath, "save.png"))
        ...
        settingsphoto = tk.PhotoImage(file=os.path.join(exedirpath, "settings.png"))
    ...
    def intensity_graph_frame(subframe21):
        ...
        #ax1 = fig1.add_subplot(111)
        #fig1.tight_layout()
        axes[0].set_xlabel("X (px)", fontsize=14)
        axes[0].set_ylabel("Intensity (arb.units)", fontsize=14)
        
        canvas1 = FigureCanvasTkAgg(fig, master=subframe211)
        #canvas1.get_tk_widget().grid(column=1, row=1, sticky="nswe")
        #canvas1._tkcanvas.grid(column=1, row=1, sticky="nswe")
        canvas1.get_tk_widget().pack(side="top", fill='both', expand=True)
        #canvas1.pack(side="top",fill='both',expand=True)
        ...
        #fig2.tight_layout()
        axes[1].set_xlabel("Y (px)", fontsize=14)
        axes[1].set_ylabel("Intensity (arb.units)", fontsize=14)
    ...
    def plotter():
        ...
        if state == 0:
            axes[0].plot(X.index.values, X.values)
            axes[0].set_xlabel("X (px)")
            axes[0].set_xlim(0,cam_res_w)
            axes[0].set_xticks(np.arange(0,cam_res_w+1,100))
        elif state == 1:
            realsize_X = CV2.pixel_to_realsize(X, "mm")
            axes[0].plot(realsize_X, X.values)
            axes[0].set_xlabel("X (mm)")

            axes[0].set_xlim(0,realsize_X[cam_res_w-1])
        ...
        axes[0].set_yticks(np.arange(0,1.2+0.2,0.2))
        axes[0].tick_params(labelsize=14)
        
        YY = Y.iloc[::-1]
        if state == 0:
            #ax2.plot(YY.values, Y.index.values)
            axes[1].plot(Y.index.values, Y.values)
            axes[1].set_xlabel("Y (px)",labelpad=None)
            axes[1].set_xlim(0,cam_res_h)
            axes[1].set_xticks(np.arange(0,cam_res_h+1,100))
        elif state == 1:
            realsize_Y = CV2.pixel_to_realsize(YY, "mm")
            realsize_Y.reverse()
            #ax2.plot(YY.values, realsize_Y)
            axes[1].plot(realsize_Y, Y.values)
            axes[1].set_xlabel("Y (mm)",labelpad=None)
            axes[1].set_xlim(0,realsize_Y[cam_res_h-1])
            ...
            #ax2.plot(YY.values, realsize_Y)
            axes[1].plot(realsize_Y, Y.values)
            axes[1].set_xlabel("Y (um)",labelpad=None)
            axes[1].set_xlim(0,realsize_Y[cam_res_h-1])
            #ax2.set_xlim(0,realsize_Y[0])
        axes[1].set_ylabel("Intensity (arb.units)",labelpad=None)
        axes[1].set_ylim(0,1.2)
        axes[1].set_yticks(np.arange(0,1.2+0.2,0.2))
    ...
    def switch_tracking():
        ...
        if tracking_state == 1:
            ...
            varh.set(False)
            varv.set(False)
            trackingphoto = tk.PhotoImage(file=os.path.join(exedirpath, "tracking_auto.png"))
            ...
            varh.set(True)
            varv.set(True)
            trackingphoto = tk.PhotoImage(file=os.path.join(exedirpath, "tracking_manual.png"))
            trackingbutton.configure(image=trackingphoto)
            trackingbutton.image = trackingphoto
