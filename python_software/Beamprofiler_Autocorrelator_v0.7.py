...
class CV2:
    ...
    def beam_size(dr, dc, dr_peak, dc_peak, beam_width):  #ビームサイズ算出(px)
        ...
        dr_cut_1 = dr[dr_peak-1:cam_res_w]
        dc_cut_0 = dc[0:dc_peak-1]
        dc_cut_1 = dc[dc_peak-1:cam_res_h]
    ...
    def pixel_to_realsize(pixel_data):
        realsize = []
        for i in pixel_data.index.values:
            realsize_data = CV2.from_pixel_to_beam_width(i)
    ...
    def knifeedge():
        test = cv2.imread("D:\Downloads\shutterspeed_test_100.jpg", 0)
        test_sum = np.sum(test)
        test_sum_columns = np.sum(test, axis=0)
        columns_sum = []
        columns_value = []
        for i in np.arange(0, len(test_sum_columns), 1):
            columns_value = np.append(columns_value, test_sum_columns[i])
            columns_sum = np.append(columns_sum, np.sum(columns_value))
        columns = []
        columns_list = []
        for i in np.arange(0, len(columns_sum)-1, 1):
            columns = columns_sum[i+1] - columns_sum[i]
...
class GUI:
    def setup():
        ...
        horizontalslider = ttk.Scale(mainframe, from_=0, to=cam_res_w-1, length=490, orient="h")
        horizontalslider.place(x=350, y=450)
        
        verticalslider = ttk.Scale(mainframe, from_=cam_res_h-1, to=0, length=390, orient="v")
    ...
    def plotter():
        ...
        if state == 0:
            ...
            realsize_X = CV2.pixel_to_realsize(X)
...
class GUI_menu:
    ...
    def settings():
        ...
        global exposuretime, exposuretimebox
        global cam_res_box
        global pixelsizebutton, actualsizebutton, var
        global varh, varv
        ...
        actualsizebutton = ttk.Radiobutton(t2frame1, text="Actual size", variable=var, value=1, command=GUI_menu.set_actualsize())
        actualsizebutton.grid(row=3, column=1, sticky="w")
    ...
    def set_actualsize():
        ...
        state = var.get()
        if state == 0:
            #actualbutton.configure(state="disabled")
            pass
        elif state == 1:
            #pixelsizebutton.configure(state="disabled")
            pass
