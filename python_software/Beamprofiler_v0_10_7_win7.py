...
class GUI:
    def setup():
        ...
        #mainframe_.grid_columnconfigure(2, weight = 1)
    ...
    def resize_window(slider):
        ...
        if root.winfo_width() <= 890:
            ...
            #subframe.grid(row=1, column=2, sticky="nswe")
            width = int(640/4*3*1.3)
            height = int(480/4*3*1.3)
            if resize != 7:                    
                GUI.change_scale()
                if resize == 0:
                    GUI.change_layout_()
    ...
    def window_configure():
        ...
        profile4 = root.after(1000, GUI.window_configure)
    ...
    def intensity_graph_frame(subframe21):
        ...
        fig1 = Figure(figsize=(8, 3), dpi=65)
        fig1.subplots_adjust(bottom=0.2)
        ax1 = fig1.add_subplot(111)
        #fig1.tight_layout()
        ...
        fig2 = Figure(figsize=(8, 3), dpi=65)
    ...
    def beamprofiler_img():
        ...
        profile1 = imgcap.after(100, GUI.beamprofiler_img, cam)
    ...
    def plotter():
        ...
        profile2 = root.after(100, GUI.plotter)
    ...
    def beam_width():
        ...
        profile3 = root.after(100, GUI.beam_width)
    ...
    def exposure():
        ...
        profile5 = root.after(100, GUI.exposure)
    ...
    def gain():
        ...
        profile6 = root.after(100, GUI.gain)
...
class GUI_menu:
    def mainmenu():
        ...
        cam_select.add_command(label="Refresh", command=GUI.cam_refresh)
        cam_list = GUI.cam_select()
        for i in cam_list:
            cam_select.add_command(label="%d" % i, command=GUI_menu.switch_cam(i))
    ...
    def switch_cam(camera_id):
        def x():
            #cam.release()
            #root.after_cancel()
            cam = GUI.cam_setup_test(camera_id)
            return cam
        return x
...
if __name__ == "__main__":
    ...
    if "Windows-10" in platform.platform():
        profile1 = root.after(0, GUI.beamprofiler_img)
        ...
        multith.setDaemon(True)
        multith.start()
    profile2 = root.after(0, GUI.plotter)
    profile3 = root.after(0, GUI.beam_width)
    #root.after(0, GUI.graph_resize_test)
    profile4 = root.after(0, GUI.window_configure)
    profile5 = root.after(0, GUI.exposure)
    profile6 = root.after(0, GUI.gain)
